  ...
  return (
    <div className="authentication">
      <div className="authentication-form card p-3">
        <h1 className="card-title">Welcome to MedConsult</h1>
        <Form layout="vertical" onFinish={onFinish}>
          <Form.Item label="Name" name="name" required rules={[ {
            message: 'name should contain only alphabits',
            validator: (_, value) => {
              if (/^[a-zA-Z\s]+$/.test(value)) {
                return Promise.resolve();
              } else {
                return Promise.reject('name field is rejected');
              }
            }
          }]}>
          ...
          <Form.Item label="Password" name="password" required rules={[{
            ...
            validator: (_, value) => {
              if (/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/.test(value)) {
                ...
                return Promise.reject('password field is rejected');
              }
            }
            }]}>
